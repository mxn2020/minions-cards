**MINIONS CARDS — KNOWLEDGE CARDS**

You are tasked with creating the complete initial foundation for **minions-cards** — atomic structured knowledge with epistemic tracking. This is part of the Minions ecosystem, a universal structured object system designed for building AI-native tools.

---

**PROJECT OVERVIEW**

`minions-cards` is a structured system for building atomic knowledge card decks with spaced repetition, confidence tracking, and source verification. Think Anki meets a knowledge graph.

Cards are atomic units of knowledge — each one captures a single fact, concept, or relationship. They can be linked, reviewed, and their confidence decays naturally over time without reinforcement.
...
1. **`card`** — An atomic knowledge unit
   - Fields: `question` (textarea, required), `answer` (textarea, required), `category` (select), `difficulty` (select: easy/medium/hard), `confidence` (number, 0-1), `lastReviewed` (date), `nextReview` (date), `interval` (number), `easeFactor` (number)
   - Relations: `references` → source, `depends_on` → prerequisite cards

2. **`source`** — The origin of a knowledge claim
   - Fields: `name` (string, required), `url` (url), `sourceType` (select: paper/book/documentation/experience), `credibility` (number, 0-1), `publishedAt` (date)
   - Relations: `parent_of` → cards

3. **`confidence-score`** — A point-in-time confidence measurement
   - Fields: `score` (number, required, 0-1), `measuredAt` (date, required), `context` (textarea), `method` (select: self-assessment/test/peer-review)
   - Relations: `references` → card

4. **`review-session`** — A batch review of cards
   - Fields: `startedAt` (date, required), `completedAt` (date), `cardsReviewed` (number), `accuracy` (number, 0-1), `duration` (number)
   - Relations: `references` → cards reviewed

---

**BEYOND STANDARD PATTERN**

- Spaced repetition scheduling (SM-2 algorithm) on `review-session` minions
- Confidence decay over time without review
- `CardGraph` linking cards via typed knowledge relations
...
```bash
cards new "What is RAG?" --answer "Retrieval Augmented Generation..."
cards review
cards due
cards confidence <id>
cards link <id-a> <id-b> --type "prerequisite"
cards export --format anki
```
...
Agents can build, maintain, and query structured knowledge bases with confidence-aware retrieval. Knowledge that isn't reinforced naturally fades.
...
Complete Python port with identical functionality. Published to PyPI as `minions-cards`.
